...
Run only after fresh verification. When the active workflow authorizes commits, commit the artifact with the work before `request-review`; otherwise leave it in the working tree, which `request-review` includes alongside committed changes. Never invoke this skill from a hook, and never create extra artifacts for nested workflow steps.
